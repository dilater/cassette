...
A complete spec for Codex to build a personal media player on Windows. Working name "Cassette", rename freely.

---

## 0. How to use this document

Save this file as `AGENTS.md` at the root of your project once you've scaffolded the repo. Codex reads `AGENTS.md` automatically and uses it as long-term context.
...
| Frontend framework | React 18 + TypeScript + Vite | Industry default, Codex is comfortable with it, no surprises. |
...
Each phase ends with a working, demoable state. Don't move to phase N+1 until phase N runs end-to-end. Update this section as phases land so future-you (or future-Codex-session) knows where you are.
...
```
cassette/
├── AGENTS.md                        # this file, kept up to date
├── README.md
├── package.json
├── tsconfig.json
├── vite.config.ts
...
```css
:root {
  --bg-base: #0c0c0c;                       /* outermost surface */
  --bg-frame: #161616;                       /* video frame fallback when no video */
...
- Two weights only: 400 (regular) and 500 (medium). Never 600+. Heavy weights look out of place against the host UI.
- Sentence case throughout. Never Title Case, never ALL CAPS. "Up next: episode 5", not "Up Next: Episode 5".
- No em dashes anywhere in UI strings, code comments, file paths, or this document. Use commas, middle dots, parentheses, or colons. This is a hard rule. Add an ESLint rule to enforce it on TS, and check in pre-commit for Rust.
- Sizes: 11px (smallest, mono labels), 12px (body), 13px (page headers like "Library"). Never go below 11px.
- Letter-spacing: 0.04em on small caption labels like "Continue watching", "Browse", "Watched folders".
- Mono font is for technical readouts: timecodes, filenames, GPU stats, file paths, episode numbers.

### Layout primitives

- Outer player frame: 12px radius, 0.5px border, dark base background. This is the only place a hard border lives.
- Title bar: 12px vertical padding, three small dot indicators on the left, content (filename or page title) at 12-13px, 0.5px bottom border at very low opacity.
- Video frame: 16:9 aspect, transparent (so the GL surface shows through) when video is playing, dark fallback color when nothing is loaded.
- Transport section: 14px / 18px padding around scrubber and controls.
- Chips on video: 6px / 11px padding, 999px radius, dark translucent background, no border.
...
- **Hover affordance**: bare icons that brighten and gain a soft circular halo on hover. No fixed button outlines. Idle icon at 70% opacity, hovered at 100% with a subtle white wash background.
- **The center play button is the one solid white circle in the entire app.** It carries the primary action; let it have that weight. Everything else fades into the chrome.
...
### Things Codex should not do (torrent module)
